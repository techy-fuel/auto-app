...
const statusTone = { Available: 'emerald', Reserved: 'amber', Sold: 'slate', 'In Service': 'blue' };
...
function statusFor(vehicle) {
  if (vehicle.status === 'Available' && vehicle.price > 0) return 'Available';
...
export function Inventory() {
  ...
  const lowAlert = vehicles.filter(v => v.status === 'Available').length < 5 ? vehicles.filter(v => v.status === 'Available').length : 0;
  ...
  return (
    <div style={{ padding: 'var(--space-6)', display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 'var(--space-4)' }}>
        ...
        <StatCard label="Sold This Month" value={soldCount} accent="amber"
      ...
      <AiInsight title="Inventory AI" tone={availableCount < 10 ? 'alert' : 'opportunity'}>
        {availableCount < 10
          ? `Only ${availableCount} vehicles available. Consider restocking — low inventory affects lead conversion rates.`
      ...
      {showForm && (
        <div style={{ background: 'var(--white)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-lg)', padding: 'var(--space-6)', boxShadow: 'var(--shadow-card)' }}>
          <h3 style={{ font: 'var(--weight-bold) 15px/1 var(--font-display)', color: 'var(--text-strong)', marginBottom: 'var(--space-5)' }}>Add Vehicle to Inventory</h3>
      ...
      <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border-soft)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-card)', overflow: 'hidden' }}>
        ...
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map(v => (
                <tr key={v.id} style={{ borderBottom: '1px solid var(--divider)', cursor: 'pointer' }}
                  ...
                  <td style={{ padding: '14px 16px' }}>
                    <button onClick={() => deleteVehicle(v.id)} style={{ padding: '5px 10px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-soft)', background: 'var(--white)', font: 'var(--weight-semibold) 11px/1 var(--font-display)', color: 'var(--red-500)', cursor: 'pointer' }}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ padding: 'var(--space-12)', textAlign: 'center', color: 'var(--text-muted)' }}>
            {vehicles.length === 0 ? 'No vehicles yet. Click "Add Vehicle" to get started!' : 'No vehicles match your filters.'}
          </div>
        )}
      </div>
    </div>
